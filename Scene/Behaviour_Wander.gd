class_name Behaviour_Wander extends SteeringBehavior
...
func _ready():
	boid = get_parent()
	wander_target = get_random_point_in_radius(radius)
...
func get_random_point_in_radius(radius):
...
func calculate():		
	...
	var disp = jitter * get_random_point_in_radius(radius) * delta
	wander_target += disp
